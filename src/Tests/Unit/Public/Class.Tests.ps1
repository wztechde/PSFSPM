...
Describe 'Class-Tests' -Tag Unit {
   Context 'Class FMPermission' -Tag Unit {
      It 'vGetDetailedInheritance()erify that helper function exists' {
         { New-FMPermission -Identity 'foo' -FileRight 'Modify' -inh 'ThisFolderOnly' } | Should -Not -Throw "*is not recognized"
      }
      It 'checks for correct object being returned' {
        (New-FMPermission -ide 'foo' -FileRight 'Modify' -inh 'ThisFolderOnly').GetType() | Should -Be 'FMPermission'
      }
   }#end Context
   Context 'Class FMPermission - method Get_ExplicitInheritance' -Tag Unit {
      It 'checks for members of returned object' {
         $result = New-FMPermission -Identity 'foo' -FileRight 'Modify' -Inheritance 'ThisFolderOnly'
         $result.Identity | Should -Be 'foo'
         $result.FileRight | Should -Be 'Modify'
         $result.Inheritance | Should -Be 'ThisFolderOnly'
         $result.GetDetailedInheritance().Propagate | Should -Be 'None'
         $result.GetDetailedInheritance().Inherit | Should -Be 'None'
      }
   }#end context
   Context "Class FMPermission - method Get_FileSystemAccessRule" -Tag Unit {
      BeforeAll {
         $FMP1 = New-FMPermission -Identity foo -FileRight Write -Inheritance ThisFolderAndFiles
         $FMP2 = New-FMPermission -Identity bar -FileRight Read -Inheritance ThisFolderOnly
      }
      It "checks, that method is accessible" {
         { $FMP1.GetFileSystemAccessRule() } | Should -Not -Throw
      }
      It "checks for correct object being returned FMP1" {
         $result = $FMP1.GetFileSystemAccessRule()
      ...
      It "checks for correct object being returned FMP2" {
         $result = $FMP2.GetFileSystemAccessRule()
         $result.count | Should -Be 1
   ...
   Context "Class FMPathPermission" -Tag Unit {
      It "verify that helper function exists" {
         { New-FMPathPermission -Path C:\Temp -InputObject (New-FMPermission -Id 'foo' -FileRight 'Modify' -Inh 'ThisFolderOnly') } | Should -Not -Throw "*is not recognized"
      }
      It 'checks for correct object being returned' {
         (New-FMPathPermission -Path C:\Temp -InputObject (New-FMPermission -Id 'foo' -FileRight 'Modify' -Inh 'ThisFolderOnly')).Gettype() | Should -Be 'FMPathPermission'
      }
   }#end Context
   Context 'Class FMPathPermission - paramset [InputObject]' -Tag Unit {
      It "checks correct object being returned - one FMPermission" {
         $Permission = New-FMPermission -Id 'foo' -FileRight 'Modify' -Inh 'ThisFolderOnly'
         ...
         $result.Permission.FileRight | Should -Be "Modify"
         $result.Permission.Inheritance | Should -Be "ThisFolderOnly"
      }
      It "checks correct object being returned - two FMPermission" {
         $pm1 = New-FMPermission -Id 'foo' -FileRight 'Modify' -Inh 'ThisFolderOnly'
         $pm2 = New-FMPermission -Id 'bar' -FileRight 'Read' -Inh 'File'
   ...
   Context 'Class FMPathPermission - paramset [Default]' -Tag Unit {
      It "verify that helper funtion parameterset 'default' exists" {
         { New-FMPathPermission -Path C:\Temp -Identity 'foo' -FileRight 'Write' -Inheritance 'ThisFolderOnly' } | Should -Not -Throw
      }
      It 'checks for correct object being returned' {
         (New-FMPathPermission -Path C:\Temp -Identity 'foo' -FileRight 'Write' -Inheritance 'ThisFolderOnly').Gettype() | Should -Be 'FMPathPermission'
      }
      It 'checks for correct object being returned - one path' {
         $Result = New-FMPathPermission -Path C:\Temp -Identity 'foo' -FileRight 'Write' -Inheritance 'ThisFolderOnly'
         $result.Path | Should -Be 'C:\Temp'
         $result.Permission.Count | Should -Be 1
         $result.Permission.Identity | Should -Be "foo"
         $result.Permission.FileRight | Should -Be "Write"
         $result.Permission.Inheritance | Should -Be "ThisFolderOnly"
      }
      It "checks for correct object being returned - two permissions" {
         $Result = New-FMPathPermission -Path C:\Temp -Identity foo, bar -FileRight Write, Read -Inheritance ThisFolderAndFiles, ThisFolderOnly
         $Result.Permission.Count | Should -Be 2
         $result.Permission.Identity | Should -Be ('foo', 'bar')
         $result.Permission.FileRight | Should -Be ('Write', 'Read')
         $result.Permission.Inheritance | Should -Be ("ThisFolderAndFiles", "ThisFolderOnly")
      }
   }#end context
   Context 'Class FMPathPermission - method GetFileSystemAccessRule' -Tag Unit {
      It 'checks that method is available' {
         $FMPP = New-FMPathPermission -Path C:\Temp -Identity 'foo' -FileRight 'Write' -Inheritance 'ThisFolderOnly'
         { $FMPP.Permission.GetFileSystemAccessRule() } | Should -Not -Throw
      }

      It 'Checks for correct result with one permission object' {
         $FMPP = New-FMPathPermission -Path C:\Temp -Identity 'foo' -FileRight 'Write' -Inheritance 'ThisFolderOnly'
         $result = $FMPP.Permission.GetFileSystemAccessRule()
         $result.count | Should -Be 1
         $result.FilesystemRights | Should -Be "Write, Synchronize"
         $result.IdentityReference | Should -Be "foo"
         $result.InheritanceFlags | Should -Be "None"
      ...
      It 'Checks for correct result with one permission object' {
         $FMPP = New-FMPathPermission -Path C:\Temp -Identity 'foo', 'bar' -FileRight 'Write', 'read' -Inheritance 'ThisFolderOnly', 'FilesOnly'
         $result = $FMPP.Permission.GetFileSystemAccessRule()
   ...
   Context 'Class FMPathPerimssion - method SetAccess' -Tag Unit {
      # prerequisites
      BeforeAll {
   ...
   Context "Class FMDirectory" -Tag Unit {
      ...
      BeforeAll {
         ...
         $F_Bar = mkdir "$TestDrive\foo\bar" -Force
         $F_Clara = mkdir "$TestDrive\foo\bar\clara" -Force
         $Root = New-FMPathPermission -Path $F_Foo -Identity pester2, pester3 -FileRight read, write -Inheritance ThisFolderSubfoldersAndFiles, ThisFolderSubfoldersAndFiles
         $Child1 = New-FMPathPermission -Path "bar" -Identity Pester1 -FileRight Read -Inheritance ThisFolderOnly
         $Child2 = New-FMPathPermission -Path "bar" -Identity Pester1 -FileRight Modify -Inheritance ThisFolderAndSubfolders
         $Child3 = New-FMPathPermission -Path "bar\clara" -Identity Pester1, pester2 -FileRight Write, Read -Inheritance ThisFolderOnly, ThisFolderSubfoldersAndFiles
      ...
      It "verifies that child paths with drive information fail" {
         $Child = New-FMPathPermission -Path C:\Foo -Identity Pester1 -FileRight Write -Inheritance ThisFolderAndFiles
      ...
      It "verifies that child paths with drive information fail - more children" {
         $Child = New-FMPathPermission -Path Foo -Identity Pester1 -FileRight Write -Inheritance ThisFolderAndFiles
         $Child2 = New-FMPathPermission -Path C:\Foo -Identity Pester2 -FileRight Write -Inheritance ThisFolderAndFiles
         { New-FMDirectory -Root $Root -Child $Child, $Child2 } | Should -Throw "FMDirectory - children must not contain drive information*"
      }
      It "SetAccess - check for method existing" {
         $TestDir = New-FMDirectory -Root $Root -Child $Child1
         { $TestDir.SetAccess() } | Should -Not -Throw "Method invocation fail*"
      }
      It "SetAccess - check for correct information being returned - Child1" {
         $TestDir = New-FMDirectory -Root $Root -Child $Child1
         $Result = $TestDir.SetAccess()
